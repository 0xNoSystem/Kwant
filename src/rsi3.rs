...
impl SmaOnRsi{
    fn new(smoothing_lengthlength: usize) -> Self{
       
        assert!(periods > 3, "length field must be a positive integer > 3, ({})", periods);
    ...
    fn push(&mut self, new_rsi: f32){
        ...
        if self.is_full(){
            let expired_rsi = self.pop_front().unwrap();
            self.sum -= expired_rsi;

            self.push_back(new_rsi);
        }
